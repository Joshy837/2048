...
function join(board_join, x1, y1, x2, y2) {
  //Compares two adjacent tiles
  if (board_join[x1][y1] != 0) {
    if (board_join[x2][y2] == 0) {
      board_join[x2][y2] = board_join[x1][y1];
      board_join[x1][y1] = 0;
      check_move = 1;
    } else if (board_join[x1][y1] == board_join[x2][y2]) {
      //Check if both tiles are equal
      if (
        !joined_positions.includes(x1 + n_columns * y1) &&
        !joined_positions.includes(x2 + n_columns * y2)
      ) {
        board_join[x2][y2] += 1;
        board_join[x1][y1] = 0;
...
function move(board_move, direction) {
  //Follow a specific set of actions for each individual tile comparison
  joined_positions = [];
  check_move = 0;
  let n_parallel;
  let n_perpendicular;
  switch (direction % 180) {
    case 0:
      n_parallel = n_rows;
      n_perpendicular = n_columns;
      break; //left-right direction
    case 90:
      n_parallel = n_columns;
      n_perpendicular = n_rows;
      break; //up-down direction
  }
      ...
      for (let k = 1; k < n_perpendicular - j; k++) {
        switch (direction) {
          case 0:
            ...
            break;
          case 90:
            join(board_move, k, i, k - 1, i);
            break;
          case 180:
            join(board_move, i, k, i, k - 1);
            break;
          case 270:
            join(
              board_move,
...
function new_tile(board_tile) {
  //generates new tile on the board after each move
  let empty_tile = [];
...
function display(display_board) {
